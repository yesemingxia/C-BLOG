  ...
  const renderPreview = (md: string) => {
    ...
    return lines.map((line, i) => {
      ...
      if (line.startsWith(`> `)) return (
        <blockquote key={i} className="my-3 pl-4 py-2 italic text-sm" style={{ borderLeft: `3px solid #7c6aff`, background: `rgba(124,106,255,0.07)`, borderRadius: `0 0.5rem 0.5rem 0`, color: `rgba(var(--foreground-rgb),0.75)` }}>
          {line.slice(2)}
        </blockquote>
      );
      if (line.startsWith(`- `)) return <li key={i} className="text-sm mb-1 ml-4" style={{ color: `rgba(var(--foreground-rgb),0.8)` }}>{line.slice(2)}</li>;
      if (line.trim() === ``) return <div key={i} className="h-3" />;
      return <p key={i} className="text-sm leading-relaxed mb-1" style={{ color: `rgba(var(--foreground-rgb),0.8)` }}>{line}</p>;
    });
  };
    ...
    <div data-cmp="Write" className="min-h-screen relative">
      ...
      >
          <div className="flex items-center gap-2 flex-wrap">
            {toolbarItems.map((item, i) => (
              <button
                ...
                title={item.label}
                className="w-8 h-8 rounded-lg flex items-center justify-center transition-all hover:bg-foreground/10"
                style={{ color: `rgba(var(--foreground-rgb),0.6)` }}
              >
                <item.icon size={15} />
              </button>
            ))}
            <div className="w-px h-5 mx-1" style={{ background: `rgba(var(--foreground-rgb),0.08)` }} />
            {[`H1`, `H2`, `H3`].map((h) => (
              <button
                key={h}
                onClick={() => {
                ...
                className="px-2 h-8 rounded-lg text-xs font-mono transition-all hover:bg-foreground/10"
                style={{ color: `rgba(var(--foreground-rgb),0.6)` }}
              >
                {h}
              </button>
          ...
          <div className="flex items-center gap-3">
              ...
              {[
                ...
                <button
                  ...
                  style={{
                    ...
                    color: previewMode === v.mode ? `#a78bfa` : `rgba(var(--foreground-rgb),0.5)`,
                  }}
                >
                  <v.icon size={12} />
            ...
            <div className="text-xs" style={{ color: `rgba(var(--foreground-rgb),0.3)` }}>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div
            ...
            style={{
              ...
              borderRight: previewMode === `split` ? `1px solid rgba(var(--foreground-rgb),0.06)` : `none`,
            }}
          >
            {/* Title input */}
            <div className="px-8 pt-8 pb-4" style={{ borderBottom: `1px solid rgba(var(--foreground-rgb),0.05)` }}>
              <input
                type="text"
                placeholder="✨ 输入文章标题..."
                value={title}
      ...
      >
        <div
          ...
          style={{
            ...
            borderLeft: `1px solid rgba(var(--foreground-rgb),0.08)`,
            padding: `2rem`,
        ...
        >
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={() => setShowPublishPanel(false)}>
              <X size={18} style={{ color: `rgba(var(--foreground-rgb),0.5)` }} />
            </button>
          </div>
          ...
          <div className="mb-6">
            ...
            <div
              ...
              style={{
                height: 160,
                background: coverUrl ? `transparent` : `rgba(var(--foreground-rgb),0.04)`,
                border: `1px dashed rgba(124,106,255,0.3)`,
              }}
            >
              <div className={coverUrl ? `hidden` : `text-center`}>
                <Image size={24} style={{ color: `rgba(var(--foreground-rgb),0.3)`, margin: `0 auto 8px` }} />
                <p className="text-xs" style={{ color: `rgba(var(--foreground-rgb),0.35)` }}>点击上传或输入图片链接</p>
              </div>
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {allTags.map((tag) => (
                <button
                  ...
                  style={{
                    background: selectedTags.includes(tag) ? `rgba(124,106,255,0.2)` : `rgba(var(--foreground-rgb),0.05)`,
                    color: selectedTags.includes(tag) ? `#a78bfa` : `rgba(var(--foreground-rgb),0.6)`,
                    border: `1px solid ${selectedTags.includes(tag) ? `rgba(124,106,255,0.35)` : `rgba(var(--foreground-rgb),0.07)`}`,
                  }}
                >
                  {tag}
                </button>
          ...
          <div className="mb-8">
            ...
            <div className="relative">
              <select
                ...
                style={{ color: `rgba(var(--foreground-rgb),0.8)`, background: `transparent` }}
              >
              ...
              <ChevronDown size={14} className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none" style={{ color: `rgba(var(--foreground-rgb),0.4)` }} />
            </div>
          </div>
