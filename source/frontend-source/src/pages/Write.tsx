...
const Write = () => {
  ...
  // @cuiruoni+判断颜色是否过浅（在浅色背景下会看不见），用于过滤 localStorage 中可能残留的失效旧值
  const isTooLight = (hex: string) => {
    const sanitized = hex.replace(`#`, ``);
    if (sanitized.length !== 6) return true;
  ...
  // @cuiruoni+编辑器文字颜色：默认深色前景色保证浅色背景对比度；若 localStorage 存的是过浅旧值则回退到默认
  const [editorTextColor, setEditorTextColor] = useState(() => {
    const saved = localStorage.getItem(`write-editor-text-color`);
    if (saved && !isTooLight(saved)) return saved;
    return `#0f172a`;
  });

  useEffect(() => {
    localStorage.setItem(`write-editor-text-color`, editorTextColor);
  }, [editorTextColor]);
  ...
  const renderPreview = (md: string) => {
    ...
    return lines.map((line, i) => {
      ...
      if (line.startsWith(`### `)) return <h3 key={i} className="text-lg font-bold mb-2 mt-4 text-[var(--foreground)]">{line.slice(4)}</h3>;
      if (line.startsWith(`> `)) return (
        <blockquote key={i} className="my-3 pl-4 py-2 italic text-sm border-l-[3px] border-[var(--foreground)] rounded-r-lg bg-[var(--muted)] text-[var(--muted-foreground)]">
          {line.slice(2)}
        </blockquote>
      );
      if (line.startsWith(`- `)) return <li key={i} className="text-sm mb-1 ml-4 text-[var(--muted-foreground)]">{line.slice(2)}</li>;
      if (line.trim() === ``) return <div key={i} className="h-3" />;
      return <p key={i} className="text-sm leading-relaxed mb-1 text-[var(--muted-foreground)]">{line}</p>;
    });
  };
  ...
  return (
    <div data-cmp="Write" className="min-h-screen relative">
      ...
      <div className="relative z-10 flex flex-col" style={{ paddingTop: 64, height: `100vh` }}>
        {/* Write toolbar */}
        <div className="flex-shrink-0 flex items-center justify-between px-6 py-3 bg-[var(--muted)] border-b border-[var(--border)]">
          <div className="flex items-center gap-2 flex-wrap">
            {toolbarItems.map((item, i) => (
              <button
                ...
                title={item.label}
                className="w-8 h-8 rounded-lg flex items-center justify-center transition-all hover:bg-[var(--brand-subtle)] text-[var(--muted-foreground)]"
              >
                <item.icon size={15} />
              </button>
            ))}
            ...
            {[`H1`, `H2`, `H3`].map((h) => (
              <button
                ...
                className="px-2 h-8 rounded-lg text-xs font-mono transition-all hover:bg-[var(--brand-subtle)] text-[var(--muted-foreground)]"
              >
                {h}
              </button>
            ))}
          ...
          <div className="flex items-center gap-3">
            ...
            {/* 文字颜色选择器 */}
            <div className="flex items-center gap-2" title="调整编辑器文字颜色">
              <div
                className="w-5 h-5 rounded-full border border-[var(--border)] shadow-sm"
                style={{ background: editorTextColor }}
              />
              <input
                type="color"
          ...
          >
            ...
            <div className="px-8 pt-8 pb-4 border-b border-[var(--border)]">
              <input
                ...
                style={{ color: editorTextColor, caretColor: `var(--foreground)` }}
              />
            </div>
            {/* Content textarea */}
            <textarea
      ...
      >
        <div
          className="overflow-y-auto bg-[var(--card)] border-l border-[var(--border)]"
          style={{
            width: 400,
            height: `100vh`,
            padding: `2rem`,
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-bold text-[var(--foreground)]">{editId ? `编辑文章` : `发布文章`}</h3>
            <button onClick={() => setShowPublishPanel(false)} className="text-[var(--muted-foreground)]">
              <X size={18} />
            </button>
          </div>
          ...
          {/* Cover */}
          <div className="mb-6">
            <label className="text-sm font-medium text-[var(--foreground)] mb-2 block">封面图</label>
            <div
              className="rounded-xl overflow-hidden mb-2 flex items-center justify-center border border-dashed border-[var(--border)]"
              style={{
                height: 160,
                background: coverUrl ? `transparent` : `var(--muted)`,
              }}
            >
              <div className={coverUrl ? `hidden` : `text-center`}>
                <Image size={24} className="text-[var(--muted-foreground)] mx-auto mb-2" />
                <p className="text-xs text-[var(--muted-foreground)]">点击上传或输入图片链接</p>
              </div>
          ...
          {/* Tags */}
          <div className="mb-6">
            <label className="text-sm font-medium text-[var(--foreground)] mb-2 block flex items-center gap-2">
              <Hash size={14} className="text-[var(--foreground)]" />
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs transition-all border ${
                    selectedTags.includes(tag)
                      ? "bg-[var(--brand-subtle)] border-[var(--border-strong)] text-[var(--foreground)]"
            ...
            <div className={showTagInput ? `` : `hidden`}>
              <div className="flex gap-2">
                ...
                />
                <button onClick={addCustomTag} className="btn-primary px-4 rounded-xl text-sm">
                  添加
                </button>
              </div>
            </div>
            <button
              ...
              className="flex items-center gap-1 text-xs mt-2 text-[var(--muted-foreground)]"
            >
              <Plus size={12} />
              自定义标签
            </button>
          ...
          {/* Summary */}
          <div className="mb-6">
            <label className="text-sm font-medium text-[var(--foreground)] mb-2 block">文章摘要</label>
            <textarea
          ...
          {/* Visibility */}
          <div className="mb-8">
            <label className="text-sm font-medium text-[var(--foreground)] mb-2 block">可见性</label>
            <div className="relative">
              <select
                value={visibility}
                onChange={(e) => setVisibility(e.target.value)}
                className="glass-input w-full px-4 py-3 rounded-xl text-sm appearance-none text-[var(--muted-foreground)]"
              >
                <option value="public" style={{ background: `var(--card)` }}>公开</option>
                <option value="private" style={{ background: `var(--card)` }}>仅自己可见</option>
                <option value="members" style={{ background: `var(--card)` }}>仅会员可见</option>
              </select>
              <ChevronDown size={14} className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-[var(--muted-foreground)]" />
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <button
              onClick={handleSaveDraft}
              disabled={saving}
              className="btn-ghost flex-1 py-3 rounded-xl text-sm font-medium text-[var(--foreground)]"
              style={{ opacity: saving ? 0.65 : 1 }}
            >
              {saving ? "保存中..." : "保存草稿"}
            </button>
